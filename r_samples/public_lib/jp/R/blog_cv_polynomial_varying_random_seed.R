...
cv <- rep(0, k) # Vector for storing results of polynomial fitting with CV
ncv <- rep(0, k) # Vector for storing results of polynomial fitting without CV
...
for (q in q_range){
  ...
  y <- cumsum(rnorm(360, 0, 20)) + 1000 # Generate a random walk
  ...
  d_learn <- d[1:300, ] # Learn dataset, 1:300
  ...
  newdata <- d[301:360, ] # Test dataset, 301:360
