function [alpha_smo,bias]=tjo_smo2(x_list,y_list,alpha,delta,Cmax,clength,learn_stlength,loop,kernel_choice)
% SMO（Sequential Minimal Optimization：逐次最小最適化ルーチン）の中身です。
% 詳しいアルゴリズムは赤本をご参照下さい。というか、赤本を見ないでこの中を
% いじるのは大変危険です。編集する前に必ず赤本を熟読して勉強し、理解して下さい。
...
alpha_smo=alpha;
    ...
    function output=Es(i)
        output=alpha(i)+learn_stlength*(1-(y_list(i)*tjo_margin(x_list(i),x_list,alpha,y_list,delta,clength,kernel_choice)));
    end
    ...
    function output = takeStep(i1,i2)
        ...
        eta = k11+k22-2*k12; % etaの定義を正負逆にしている
        
        if(eta>0)
            a2=alph_t2+y2*(E1-E2)/eta;
            a2=min([max([a2 L]) H]);
        else
            Lobj=gamma-s*L+L-0.5*k11*(gamma-s*L)^2-0.5*k22*L^2-s*k12*(gamma-s*L)*L-y1*(gamma-s*L); % a2=LにおけるW
            Hobj=gamma-s*H+H-0.5*k11*(gamma-s*H)^2-0.5*k22*H^2-s*k12*(gamma-s*H)*H-y1*(gamma-s*H); % a2=HにおけるW
            if(Lobj < Hobj-eps)
                a2 = L;
    ...
    function output=examEx(i2)
        ...
        if (r2 < -tol && alph_t2 < Cmax) || (r2 > tol && alph_t2 > 0)
